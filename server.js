...
// Hardcode /data if it exists (Railway volume), fallback to local
...
app.get('/api/health', (req, res) => {
  const row = db.prepare('SELECT COUNT(*) as cnt FROM stops').get();
  res.json({ ok: true, db: DB_PATH, stops: row.cnt });
});
...
// GET dashboard summary — always from DB
app.get('/api/dashboard', (req, res) => {
  checkWeeklyReset();
  const tours = db.prepare('SELECT DISTINCT tour_id FROM stops').all().map(r => r.tour_id);
  const result = {};
  tours.forEach(tid => {
    const rows = db.prepare('SELECT state, COUNT(*) as cnt FROM stops WHERE tour_id=? GROUP BY state').all(tid);
    result[tid] = {0:0,1:0,2:0,3:0};
    rows.forEach(r => { result[tid][r.state] = r.cnt; });
  });
  res.json(result);
});
...
app.listen(PORT, () => console.log(`CHM Distribution on port ${PORT} — DB: ${DB_PATH}`));
